...
export function ToastProvider({ children }) {
  ...
  return (
    <ToastContext.Provider value={toast}>
      {children}
      <div className="fixed bottom-20 right-4 z-[100] flex w-80 max-w-[90vw] flex-col gap-2 sm:bottom-4">
        {toasts.map((t) => (
          <div key={t.id} role="alert" className={`rounded-lg border-l-4 p-3 text-sm shadow-lg ${styles[t.type]}`}>
            <div className="flex items-start justify-between gap-2">
              <span>{t.message}</span>
              <button onClick={() => remove(t.id)} className="text-lg leading-none opacity-60 hover:opacity-100">×</button>
            </div>
          </div>
        ))}
      </div>
