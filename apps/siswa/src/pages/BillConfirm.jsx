                ...
                {midtrans && qrDataUrl && qrScannable && !sandboxLocal ? (
                  ...
                  <div className="w-full space-y-3 text-center">
                    <div className="rounded-xl border border-sky-200 bg-sky-50 px-4 py-4 dark:border-sky-900/50 dark:bg-sky-950/40">
                      ...
                      {/* Iframe Snap: tampilkan selalu jika ada redirect (saat kanal aktif = QR scannable) */}
                      {snapRedirectUrl ? (
                        <iframe
                          title="Midtrans QRIS"
                      ...
                      {(snapEmbedFailed || channelInactive) && (
                        <div className="mt-3 space-y-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-left text-[11px] leading-relaxed text-amber-900 dark:border-amber-900/50 dark:bg-amber-950/40 dark:text-amber-200">
                          <p>
                            {midtransHint || 'Kanal QRIS/GoPay belum aktif di Midtrans, sehingga kode QR scannable belum tersedia. Isi key Sandbox (SB-Mid-…) di Pengaturan → Metode Pembayaran, aktifkan QRIS di dashboard.sandbox.midtrans.com, settlement ke BNI 6513009817, lalu buat QR ulang.'}
                          </p>
                          <button
                            type="button"
