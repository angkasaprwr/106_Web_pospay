...
export default function Login() {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-gradient-to-br from-brand-600 to-brand-800 p-6">
      <button onClick={toggle} className="absolute right-4 top-4 rounded-lg bg-white/15 p-2 text-white">
      ...
      <div className="mb-6 flex flex-col items-center text-white">
        <div className="mb-3 flex h-16 w-16 items-center justify-center rounded-2xl bg-white/15"><Icon.School width={32} height={32} /></div>
      ...
      <div className="w-full max-w-sm card p-6">
        ...
        <form onSubmit={submit} className="mt-6 space-y-4">
          <div>
            ...
            <input className="input" value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })} placeholder="2025001" autoFocus required />
          </div>
          <div>
          ...
          <button className="btn-primary w-full" disabled={loading}>{loading ? <Spinner size={18} className="text-white" /> : 'Masuk'}</button>
        </form>
