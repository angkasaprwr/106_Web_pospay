    ...
    <div className="space-y-6">
      <div className="rounded-xl border border-slate-100 bg-white dark:border-slate-800 dark:bg-slate-900 p-5 shadow-sm">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 className="text-base font-semibold text-slate-900 dark:text-white dark:text-white">Filter Laporan Pembayaran</h2>
            <div className="mt-3 flex flex-wrap items-end gap-2">
              <div>
                <label className="mb-1 block text-xs text-slate-500">Jenis Periode</label>
                <select
                  className="rounded-lg border border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800 px-3 py-2 text-sm outline-none focus:border-pospay"
          ...
          <div className="relative" ref={exportRef}>
            ...
            {exportOpen && (
              <div className="absolute right-0 z-10 mt-1 w-44 overflow-hidden rounded-lg border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800 py-1 shadow-lg">
            ...
            <div className="xl:col-span-2">
              <div className="rounded-xl border border-slate-100 bg-white dark:border-slate-800 dark:bg-slate-900 shadow-sm">
                <div className="border-b border-slate-100 dark:border-slate-800 p-5">
                  <h3 className="font-semibold text-slate-900 dark:text-white dark:text-white">Rincian Pemasukan</h3>
                </div>
                {rows.length === 0 ? (
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {rows.map((r, i) => (
                          <tr key={`${r.period}-${r.tagihan}`} className="border-b border-slate-50 dark:border-slate-800 hover:bg-slate-50/50">
                        ...
                        {totalRow && rows.length > 0 && (
                          <tr className="border-t-2 border-blue-100 bg-blue-50/60 font-semibold text-slate-900 dark:text-white dark:text-white">
                ...
                {rows.length > 0 && (
                  <p className="border-t border-slate-100 px-4 py-3 text-sm text-slate-500 dark:text-slate-400">
            ...
            <div className="space-y-4">
              <div className="rounded-xl border border-slate-100 bg-white dark:border-slate-800 dark:bg-slate-900 p-5 shadow-sm">
                <div className="mb-4 flex items-center justify-between">
                  <h3 className="font-semibold text-slate-900 dark:text-white dark:text-white">Grafik Pemasukan per Bulan</h3>
                ...
                {stats && (
                  <div className="mt-4 flex items-center gap-3 rounded-xl border border-slate-100 bg-slate-50/80 p-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-lg font-bold text-slate-900 dark:text-white">{stats.totalIncomeFormatted}</p>
